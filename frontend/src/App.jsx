...
function MD({ content }) {
  ...
  while (i < lines.length) {
      ...
      elements.push(
        <div key={i} className="table-wrap">
          <table className="md-table">
            <thead><tr>{headers.map((h,j)=><th key={j}><Inline text={h}/></th>)}</tr></thead>
            <tbody>{rows.map((row,ri)=><tr key={ri}>{row.map((cell,ci)=><td key={ci}><Inline text={cell}/></td>)}</tr>)}</tbody>
          </table>
        </div>
      );
      continue;
    ...
    if (line.startsWith("## ")) { elements.push(<h2 key={i} className="md-h2"><Inline text={line.slice(3)}/></h2>); i++; continue; }
    if (line.startsWith("### ")) { elements.push(<h3 key={i} className="md-h3"><Inline text={line.slice(4)}/></h3>); i++; continue; }
    if (line.startsWith("# ")) { elements.push(<h1 key={i} className="md-h1"><Inline text={line.slice(2)}/></h1>); i++; continue; }
    if (line.match(/^[-*] /)) { elements.push(<li key={i} className="md-li"><Inline text={line.slice(2)}/></li>); i++; continue; }
    if (line.match(/^\d+\. /)) { elements.push(<li key={i} className="md-oli"><Inline text={line.replace(/^\d+\.\s/,"")}/></li>); i++; continue; }
    if (line.trim() === "") { elements.push(<div key={i} className="md-spacer"/>); i++; continue; }
    elements.push(<p key={i} className="md-p"><Inline text={line}/></p>);
    i++;
  }
...
function AuthPage({ onLogin }) {
  ...
  const doSendOTP = async () => {
    ...
    setLoading(false);
    if (!r.ok) { setErr(d.detail); return; }
    setMsg(`OTP sent! For demo, your OTP is: ${d.otp}`);
    setMode("otp");
  };
      ...
      <div className="auth-card">
        ...
        {mode==="login" && <>
          ...
          <div className="auth-links">
            <span onClick={()=>{setMode("forgot");setErr("");}}>Forgot password?</span>
            <span onClick={()=>{setMode("register");setErr("");}}>Create account</span>
          </div>
        </>}
        ...
        {mode==="register" && <>
          ...
          <div className="auth-links">
            <span onClick={()=>{setMode("login");setErr("");}}>Already have an account? Login</span>
          </div>
        </>}
        ...
        {mode==="forgot" && <>
          ...
          <button className="auth-btn" onClick={doSendOTP} disabled={loading}>{loading?"Sending…":"Send OTP"}</button>
...
export default function App() {
  const [user, setUser] = useState(()=>{
    const s = localStorage.getItem("myai_user");
  ...
  // Save chats to server on change
  useEffect(() => {
  ...
  const deleteChat = (id, e) => {
    ...
    if (activeChatId===id) {
      ...
      else { const nc=makeNewChat(); setChats([nc]); setActiveChatId(nc.id); }
    }
  };
  ...
  const pinChat = (id, e) => {
    ...
    setChats(p=>p.map(c=>{
      if (c.id!==id) return c;
      if (!c.pinned && pinned>=3) { alert("Max 3 chats can be pinned!"); return c; }
  ...
  const sendMessage = async () => {
    ...
    if (text && isImageReq(text)) {
      ...
      updateActive(c=>({...c, title:text.slice(0,30)+"…", messages:[...c.messages,{role:"user",content:text},{role:"assistant",content:`🎨 Generating: **${prompt}**`,imagePrompt:prompt}]}));
      setInput("");
      return;
    }
    ...
    setChats(p=>p.map(c=>{
      ...
      const title = !first ? userContent.slice(0,32)+"…" : c.title;
      newMsgs = [...c.messages, userMsg, {role:"assistant",content:""}];
    ...
    const apiMsgs = [...(chats.find(c=>c.id===activeChatId)?.messages||[]), userMsg]
    ...
    try {
      const res = await fetch(`${API}/chat`,{
        ...
        body:JSON.stringify({messages:apiMsgs, use_search:webSearch, search_query:webSearch?text:""})
      });
      ...
      while(true){
        ...
        for(const line of decoder.decode(value).split("\n")){
          if(line.startsWith("data: ")){
            ...
            if(d==="[DONE]") break;
            try{ full+=JSON.parse(d).text; updateActive(c=>({...c,messages:c.messages.map((m,i)=>i===c.messages.length-1?{...m,content:full}:m)})); }catch{}
          }
        }
      }
      speak(full);
    } catch {
      updateActive(c=>({...c,messages:c.messages.map((m,i)=>i===c.messages.length-1?{...m,content:"Error connecting to backend."}:m)}));
  ...
  const starters = ["Explain machine learning simply","Write a Python web scraper","Draw a futuristic city","Compare React vs Vue in a table"];

  return (
    <div className="app">
      {shareToast && <div className="toast">✅ Chat copied to clipboard!</div>}
          ...
          {pinnedChats.map(c=>(
            <div key={c.id} className={`history-item ${c.id===activeChatId?"active":""}`} onClick={()=>setActiveChatId(c.id)}>
              ...
              <span className="hi-actions">
                ...
                <button title="Delete" onClick={e=>deleteChat(c.id,e)}>🗑</button>
              </span>
            </div>
          ))}
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div style={{flex:1}}/>
          {webSearch && <span className="badge search-badge">🔍 Search ON</span>}
          {speaking && <button className="stop-btn" onClick={()=>{window.speechSynthesis.cancel();setSpeaking(false);}}>⏹ Stop</button>}
          <button className="icon-btn" title="Share chat" onClick={shareChat}>🔗</button>
        </header>
          ...
          {isEmpty?(
            <div className="welcome">
              ...
              <div className="starters">
                {starters.map((s,i)=><button key={i} className="starter-btn" onClick={()=>{setInput(s);textareaRef.current?.focus();}}>{s}</button>)}
              </div>
            </div>
          ):(
            <div className="messages">
              {messages.map((msg,i)=>(
                <div key={i} className={`message ${msg.role}`}>
                  <div className="avatar">{msg.role==="user"?user.name[0].toUpperCase():"AI"}</div>
